...
import { FacetCutAction, getSelectors } from "../deployUtils"
...
export interface DeploymentRecord {
  ...
  facets?: { [facetName: string]: { address: string; selectors: string[] } }
  deployedAt?: number
}
...
export class Deployer {
  ...
  public async getDeployedInterface(contractName: string, aliasName: string): Promise<any> {
    if (!(aliasName in this.deployedContracts)) {
      throw `${aliasName} has not yet been deployed`
    }
    return this.e.getContractAt(contractName, this.deployedContracts[aliasName].address)
  }
