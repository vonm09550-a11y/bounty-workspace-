...
| 2.3a | Companion wallet pull | GMGN `activity` (all types) for `4hQZ1GtLTvAzetszCcWrb8zxiuVvEt85Bb7y6VNzA2ve` (~1K trades + ~8K transfers) and Helius parse of its signatures, so 2.3 can reconcile inventory across both wallets | ~500 GMGN pages + ~100 Helius calls | ~15 min | `activity_companion`, `transfers_companion` tables | pulled |
