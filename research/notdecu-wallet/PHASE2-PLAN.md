...
|------|-------|---------------------|-------------|
| A. Breadth | every trade the wallet ever made (~164K rows, ~120K tx), analysed month by month over the last 12 months (2025-09-14 → 2026-09-14), earlier months as context only | what the *system* is: cadence, sizing, router, exits, quote assets, drift over time | GMGN activity pages + Helius parse |
| B. Selection | every distinct token he took a position in during the 12 months (est. 20K–40K) | what he *picks*: launchpad, age, mcap, creator, quality signals, and which picks made the money | 1 GMGN `token info` per token |
| C. Depth | ~300 hand-picked positions (top 150 winners, 75 worst losers, 75 random) + ~50 recurring creators | *how* he enters and exits: seconds after launch, position in slot, who follows him, chart shape, holder structure, dev history | kline, holder analysis, dev_score, Helius slot data |
...
|---|-----------|--------------|--------------|-----------|--------------------|--------------|
| 2.0 | Scope probe | Walk 100 GMGN activity pages to measure rows/page, cursor depth (can it reach 12 months back?), row types; parse 10 Helius batches and read credit usage from the Helius dashboard; fix month boundaries | ~100 GMGN pages (w3) + 10 Helius calls | 15 min | `plan.md` numbers confirmed, `data/probe/` | cursor reaches ≥ 12 months, else fall back to per-token walks |
| 2.1 | Full index pull | Resumable walk of `activity --type buy --type sell` back to wallet creation, then `transferIn/transferOut/add/remove`; load to DuckDB; integrity check against `stats` 7d/30d counts and `profits all` totals | ~8,200 pages (w3) at 0.4 s | ~60 min, resumable | `data/activity/*.jsonl` (gitignored if > 50 MB), `data/notdecu.duckdb` summaries, `01-index.md` monthly counts | row counts reconcile within 2% |
