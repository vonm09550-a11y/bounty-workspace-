...
const HELP = `Usage: node scripts/build-sell-amm-tx.mjs [options]

Post-graduation AMM sell.

Provide either --pool or --mint (mint derives pool on-chain via bonding curve).

Required:
  --user <PUBKEY>
  --amount <int>        token smallest units if mode=base; lamports if mode=quote

Optional:
  --mode <base|quote>   Default base. base = sell fixed token amount (sellBaseInput); quote = target SOL out in lamports (sellQuoteInput)
  --pool <PUBKEY>
  --mint <PUBKEY>
...
async function main() {
  ...
  if (values.help) exitWithHelp("build-sell-amm-tx.mjs", HELP);

  const user = requirePublicKey("--user", values.user);
  const mode = (values.mode ?? "base").toLowerCase();
  if (mode !== "base" && mode !== "quote") {
    throw new Error('--mode must be "base" or "quote"');
  }
  ...
  if (poolStr) {
    ...
    if (!bondingCurve.complete) {
      throw new Error(
        "Coin is not graduated (bondingCurve.complete is false). Use build-sell-bonding-tx.mjs.",
      );
    }
  ...
  let sdkInstructions;
  if (mode === "base") {
    sdkInstructions = await PUMP_AMM_SDK.sellBaseInput(
      swapState,
      amountBn,
      slippage,
    );
  } else {
    sdkInstructions = await PUMP_AMM_SDK.sellQuoteInput(
      swapState,
      amountBn,
      slippage,
    );
