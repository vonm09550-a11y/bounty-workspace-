...
| 2 | 三道硬闸（量 / 深度 / 安全）+ 方向执行闸 | `token info` + `token security`（安全优先交 `gmgn-contract-dd`） | 硬闸任一不过 → **不下单，出「不建议买入」**；方向不利 → 照常出订单卡并标注**执行降级** |
...
### Step 2 — 三道硬性闸门（量 / 深度 / 安全）+ 一道执行闸（方向）
...
**出口**：三道硬闸（量 / 深度 / 安全）全过 → 进 Step 3；**硬闸**任一项判"不通过"或"数据缺失" → **不下单**，按 `## Display Templates` 出「不建议买入」。硬闸每项都要落成 `通过 / 不通过 / 数据缺失` 三态之一，**数据缺失按不通过处理**——拿不到检测结果不等于没问题。

**方向与波动是第四项，但它不否决这一单**，落成 `通过 / 执行降级 / 不通过` 三态：只有 5 分钟回撤 ≥10% 判不通过（市价单在那种行情里必然失败），其余不利行情一律判执行降级——照常进 Step 3 出订单卡，但必须写出降级后的执行方式。方向数据整体拿不到时按最严一档降级（不许市价单），**不判不通过**。
...
- **方向与波动**（`thresholds.md` 三、）：**这一闸是「量的方向」，和第一闸的「量的大小」是两件事，而它判的是执行方式、不是生死。** 5 分钟回撤 ≥10% 判不通过（唯一硬停）；跌 5%~10%、或 5m/1h/24h 三窗口卖压全大于买盘，判执行降级并给出限价触发价 / 等待条件 / 减小金额中至少一个。**方向侧不产生否决计数**，不并入安检那条「≥2 项警告判不通过」。两头的实测出处都要记住：robinhood 上的 MEME 24h 成交 $63.14M、1h 32,446 笔过了量闸，而它 5 分钟内跌了 16.5%、四窗口净流向全负，那是硬停该拦的；BSC 的 VC 安全 91.4「可买」、深度 $91,442、LP 95% 已销毁，只是 5m −8.2% 加三窗口全负，那是降级而不该否决。
...
**方向判执行降级时，确认要拆成两问**：① 确认这一单（币、金额、滑点），② 确认接受降级后的执行方式。**不能把降级混在整单确认里一起过**——用户点头买这个币，不等于他知道自己买的不再是市价单。他坚持要市价单就按"用户已知悉并豁免"路径走，把豁免项显式列出来。移交限价单时执行方式随参数一起交给 `gmgn-swap`（`order strategy create --order-type limit_order --sub-order-type buy_low --check-price <触发价>`）。

- 订单四件套：买入金额（**用户原话的数额与币种，不替他换算或调整**）、滑点、防夹（默认开）、预估到手数量与价格冲击。
- **执行方式**：方向闸判"通过"时是市价单，卡里不必多写；判"执行降级"时必须写出降级后的执行方式——限价单@触发价（触发价按 `thresholds.md` 三、的公式从 `price.price` 与 `price.price_5m` 推导，并标明是推导值）、建议等待（写清等什么条件收敛）、减小金额，三者至少给一个，降级理由与读到的数写进第 7 节。**降级不改滑点公式**——`|5 分钟涨跌幅| × 0.5` 那一项已经在吃波动了。
...
- **判定，放第一行。** 三道硬闸全过 → 请用户确认（方向判降级时同一行点明建议改限价单还是等待）；**硬闸**任一项不过 → 不建议买入。不要把"虽然 X 不达标但 Y 很好"写成建议，也不要因为行情不好就把一个安全干净的币写成"不建议买入"。
...
- **四项检测各自的结果与读到的数**：三道硬闸——交易量（`price.volume_24h` / `swaps_1h` / 量池比）、可交易深度（`pool` 单边口径 + 锁仓三态）、安全；加方向与波动的执行结论（5 分钟涨跌幅 + 逐窗口净流向 → 市价 / 执行降级 / 硬停）。**安全那一格要写清结论是谁给的**——`gmgn-contract-dd` 的 0–100 分，还是本技能兜底算的（兜底就必须写"未经 gmgn-contract-dd 复核"）。
- **持有人数与 Top10 占比**（`holder_count` / `top_10_holder_rate`），以及命中的警告项各是多少（`stat.*` / `wallet_tags_stat.*`）。三位数持有人配百万市值要点名说是空壳信号。
- **订单四件套**：买入金额（用户原话的数额与币种）、滑点（按 `thresholds.md` 五、推导，**要写出波动项贡献了多少**）、防夹开关、预估到手数量与价格冲击。方向判降级时再加**执行方式**：限价单@触发价（写出触发价怎么推的）/ 建议等待（写出等什么）/ 减小金额。
...
| # | 节 | 用什么块 | 什么情况下才能省 |
|---|---|---|---|
| 1 | *(无标题)* 判定 | 一到两行：**通过，请确认** / **通过，但建议改限价单（或等待）** / **不通过** + 一句话理由 | 永不 |
| 2 | 代币与合约 | 表格：符号 / 链 / **合约全址** / 市值（FDV）/ 持有人（Top10 占比）/ 创建时间 | 永不 |
| 3 | 同名候选 | 散文，最多两句：精确匹配几个、凭什么锁定这一个、排除与折叠了几个 | 用户直接给了合约地址（那时改写一行"地址由用户给定，未做消歧"） |
| 4 | 闸门 | 表格：项 / 结果 / 读到的数 —— 四行：交易量、可交易深度、安全（三道硬闸，结果只写通过 / 不通过 / 数据缺失）、方向与波动（结果写通过 / 执行降级 / 不通过） | 永不 |
...
| 8 | 确认请求 | 一行；方向判执行降级时拆成两问：① 确认这一单 ② 确认接受降级后的执行方式 | 未通过筛选时改成"用户看到理由后自己决定是否坚持" |

不要重排、不要合并、不要发明第九节。**方向判执行降级不算未通过筛选**——八节全出，第 5 节照常摆订单参数，执行方式写进第 5 节、降级理由与建议动作写进第 7 节，第 1 节的判定仍然是"通过"。**未通过筛选时保留 1、2、3、4、7 节**，第 5、6 两节整节省略——没有订单就不要摆出订单的样子；第 1 节要点名是哪一项不过、读到的数是多少，其余项照常写"通过"。也不要提供"要不要降低标准"的台阶：用户看到理由后自己决定是否坚持，他坚持就按"用户已知悉并豁免"路径走完确认流程。
...
2. **筛选不过就不下单。** 门槛是拒绝理由，不是参考分。任一硬性项不达标，直接报告并终止；不要"虽然 X 不达标但 Y 很好所以建议买入"。**硬性项只有量、深度、安全三道**——方向与波动不是硬性项，除了 5 分钟回撤 ≥10% 那一条硬停，它改变执行方式而不否决这一单（`thresholds.md` 三、）。反过来也不许绕：把执行降级悄悄写成"通过"、或者不给降级动作就直接摆一张市价单，同样违规。
