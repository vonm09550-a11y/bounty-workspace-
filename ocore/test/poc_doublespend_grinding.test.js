...
/// set up real SQLite with the exact ocore schema from initial-db/byteball-sqlite.sql
function setupDb() {
...
/// insert a unit into the db in its pre-resolution state
...
test('full double-spend resolution: attacker grinds hash & wins via handleNonserialUnits path', t => {
	...
	/// grind tps_fee to find attacker hash < victim hash
	...
	/// both units enter the DAG as temp-bad (double-spend detected, awaiting stabilization)
	insertUnit(db, victimHash, 'temp-bad');
	insertUnit(db, attackerHash, 'temp-bad');

	/// === handleNonserialUnits (main_chain.js) ===
	/// exact query from main_chain.js, processes units in lexicographic hash order
	var rows = db.prepare(
	...
	/// process each unit in order, same as async.eachSeries in handleNonserialUnits
	rows.forEach(function(row) {

		/// === findStableConflictingUnits (main_chain.js) ===
		/// exact query from main_chain.js, finds competitors with sequence='good' sharing the same author
